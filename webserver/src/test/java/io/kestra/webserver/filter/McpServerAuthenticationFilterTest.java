...
class McpServerAuthenticationFilterTest {
    ...
    void givenPublicServer_whenServerTypeChangedToPrivate_thenAuthRequiredAfterCacheUpdate() throws InterruptedException {
        ...
        McpServer updated = new McpServer(existing.tenantId(), existing.id(), existing.description(),
            ...
            existing.enabled(), existing.isDefault(), existing.deleted(), existing.created(), existing.updated());
    ...
    void givenPublicServer_whenServerDisabled_thenServiceUnavailableWithoutAuthAfterCacheUpdate() throws InterruptedException {
        ...
        McpServer disabled = new McpServer(existing.tenantId(), existing.id(), existing.description(),
            ...
            false, existing.isDefault(), existing.deleted(), existing.created(), existing.updated());
    ...
    private String saveServer(boolean enabled, McpServer.ServerType serverType) {
        String name = "server-" + IdUtils.create();
        McpServer mcpServer = new McpServer(
            TenantService.MAIN_TENANT, name, "A test MCP server",
            null, serverType, McpServer.AuthType.BASIC, enabled, false, false, null, null
        );
