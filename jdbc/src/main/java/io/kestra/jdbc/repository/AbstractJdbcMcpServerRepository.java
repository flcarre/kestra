...
public abstract class AbstractJdbcMcpServerRepository extends AbstractJdbcCrudRepository<McpServer> implements McpServerRepositoryInterface {
    ...
    @Override
    public ArrayListTotal<McpServer> list(Pageable pageable, String tenantId) {
    ...
    public McpServer save(McpServer previousMcpServer, McpServer mcpServer) {
        ...
        McpServer toSave;
        if (previousMcpServer == null) {
            toSave = new McpServer(mcpServer.tenantId(), mcpServer.id(),
                mcpServer.description(), mcpServer.instructions(), mcpServer.serverType(), mcpServer.authType(),
                mcpServer.enabled(), mcpServer.isDefault(), mcpServer.deleted(),
                Instant.now(), Instant.now());
        } else {
            toSave = new McpServer(mcpServer.tenantId(), mcpServer.id(),
                mcpServer.description(), mcpServer.instructions(), mcpServer.serverType(), mcpServer.authType(),
                mcpServer.enabled(), mcpServer.isDefault(), mcpServer.deleted(),
                previousMcpServer.created(), Instant.now());
        }
